...
export default function BookCover({
  ...
  return (
    <section className="flex h-full min-h-[720px] items-center justify-center bg-[radial-gradient(circle_at_center,#fff7df_0%,#ead0a0_52%,#b9823d_100%)] px-4 py-10 sm:px-8">
      <button
        onClick={onOpen}
        className="group relative min-w-0 cursor-pointer outline-none transition-all duration-700 hover:-translate-y-4 hover:scale-105 active:scale-95"
      >
        {/* w-[85%] (capped at the original 420px) instead of a flat
            w-[420px] — this glow is `absolute`, so on phones it must
            shrink with the cover below it or its own blurred bounding box
            still contributes to the page's horizontal scrollable area
            even after the cover itself stops overflowing. */}
        <div className="absolute -bottom-12 left-1/2 h-24 w-[85%] max-w-[420px] -translate-x-1/2 rounded-full bg-black/35 blur-3xl transition group-hover:w-[500px] group-hover:max-w-[500px] group-hover:bg-black/45" />
